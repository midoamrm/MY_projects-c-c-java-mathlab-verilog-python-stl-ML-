...
set(handles.checkbox1, 'value',0);%discrete plant
        ...
        %discrete plant
set(handles.edit5, 'string','[0.2 0.6 0.9]');%Initializing Bp
set(handles.edit6, 'string','[0.4 -1.1 0.7]');%Initializing Ap
...
set(handles.edit12, 'string','[0.0687]');%Bm
...
function P=z_polynomial(w0, zeta, Ts)%calculates P(z)
...
function [B_stable B_unstable]=separate_B(B)%checks  the stability of B
...
fprintf(fileID,'#ifndef Subsystem_H\n');
fprintf(fileID,'#define Subsystem_H\n');
fprintf(fileID, 'extern const float Ap[');
fprintf(fileID, num2str(size(Ap,2)));
fprintf(fileID, '];\n');
fprintf(fileID, 'extern const float Bp[');
fprintf(fileID, num2str(size(Bp,2)));
fprintf(fileID, '];\n');
fprintf(fileID, 'extern const float R[');
fprintf(fileID, num2str(size(R,2)));
fprintf(fileID, '];\n');
fprintf(fileID, 'extern const float S[');
fprintf(fileID, num2str(size(S,2)));
fprintf(fileID, '];\n');
fprintf(fileID, 'extern const float T[');
fprintf(fileID, num2str(size(T,2)));
fprintf(fileID, '];\n');
fprintf(fileID,'#endif\n');
fclose(fileID);
%%%%%%%%%%
fileID2= fopen('Subsystem.c','w');
fprintf(fileID2, '#include\"Subsystem.h\"\n');

fprintf(fileID2, 'const float Ap[]= ');
fprintf(fileID2, '{ ');
for k=1:size(Ap,2)-1
fprintf(fileID2, [num2str(Ap(k)) ', ']);
end
fprintf(fileID2, [num2str(Ap(size(Ap,2)))]);
fprintf(fileID2, '};\n');

fprintf(fileID2, 'const float Bp[]= ');
fprintf(fileID2, '{ ');
for k=1:size(Bp,2)-1
fprintf(fileID2, [num2str(Bp(k)) ', ']);
end
fprintf(fileID2, [num2str(Bp(size(Bp,2)))]);
fprintf(fileID2, '};\n');
...
fprintf(fileID2, [num2str(R(size(R,2)))]);
fprintf(fileID2, '};\n');
...
fprintf(fileID2, [num2str(S(size(S,2)))]);
fprintf(fileID2, '};\n');
...
fprintf(fileID2, [num2str(T(size(T,2)))]);
fprintf(fileID2, '};\n');
fclose(fileID2);
...
arr = [];
arr = [arr get_param(strcat(name, '/T(z)'), 'handle') ];
arr = [arr get_param(strcat(name, '/Sum1' ), 'handle') ];
arr = [arr get_param(strcat(name, '/1//S(z)'), 'handle') ];
arr = [arr get_param(strcat(name, '/R(z)'), 'handle') ];

Simulink.BlockDiagram.createSubSystem(arr);


arr2 = [];
arr2 = [arr2 get_param(strcat(name, '/Bp(z)//Ap(z)'), 'handle') ];
Simulink.BlockDiagram.createSubSystem(arr2);
   
open_system(sys);

rtwbuild(strcat(name,'/Subsystem'));
rtwbuild(strcat(name,'/Subsystem1'));
%to run the .bat file
system('D:\Control Project\cmd for keil.bat')
system('D:\Control Project\cmdPlant\cmd for keil.bat')
system('D:\Control Project\cmdAll\cmd for keil.bat')
